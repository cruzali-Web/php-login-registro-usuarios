<?php
	...
	if (isset($_POST["registrar"])) 
	{
		...
		$password 	= mysqli_real_escape_string($conex, $_POST['pass']);
		$password_encrypted = sha1($password);
		$queryCheckUser = "SELECT idusuarios FROM usuarios WHERE usuario = '$usuario'";

		$resultadouser = $conex->query($queryCheckUser);
		...
		if ($filas > 0) {
			...
			// Insertar usuario
			$queryInsertUser = "INSERT INTO usuarios(NombreC, Correo, usuario, password) 
				VALUES ('$nombre', '$correo', '$usuario', '$password_encrypted')";

				$resultadoUsuario = $conex->query($queryInsertUser);
